...
func postfixBaseExpressionWithRange(tokens []tok.Token, includeRange bool) (expr ast.Expression, remainder []tok.Token, err error) {
	...
	if returnExpression, remainder, err := ReturnExpression(tokens); err == nil {
		return returnExpression, remainder, nil
	} else if err != ErrNoMatch {
		return nil, remainder, err
	}
	...
	if continueExpression, remainder, err := ContinueExpression(tokens); err == nil {
		return continueExpression, remainder, nil
	} else if err != ErrNoMatch {
		return nil, remainder, err
	}
